...
| **任务依赖 DAG** | 支持任务依赖校验、循环检测、执行批次计算与就绪态识别 |
| **团队生命周期控制** | UI 支持取消、暂停、恢复、编辑团队目标与名称 |
| **任务运维能力** | 支持编辑 pending 任务、取消 in-progress 任务、成员间转派 |
| **模板 CRUD** | 内置模板只读，自定义模板支持创建、编辑、删除 |
| **UI 直发消息** | 可从界面单播到指定成员，或广播到整个团队 |
| **Worktree 隔离** | 非 Leader 成员可自动创建独立 Git Worktree，降低并发冲突 |
| **导出 / 导入** | 可导出团队快照 JSON，导入时重建成员、任务、消息与依赖关系 |
| **层级团队预览** | 支持 `parentTeamId` 建模，子团队可继承父团队目标与近期上下文 |
| **DB 持久化** | 团队实例、成员、任务、消息、模板等状态均持久化，重启后历史完整保留 |
| **可视化追踪** | 支持任务看板、DAG 视图、团队消息流与团队侧边栏联动 |

**支持团队模板化**：预定义角色分工（如"需求分析师 + 架构师 + 前端 + 后端 + 测试"），一键启动团队实例，填入目标即可开始协作。

**当前实现状态（2026-04）**
- 已落地任务依赖 DAG、依赖校验与 claim 前置检查
- 已落地团队取消 / 暂停 / 恢复 / 更新
- 已落地任务编辑 / 取消 / 转派
- 已落地模板创建 / 编辑 / 删除
- 已落地 UI 单播 / 广播消息
- 已落地可选 Worktree 成员隔离、团队级 merge / cleanup
- 已落地团队快照导出 / 导入、子团队预览
...
│   │   ├── TeamBridge.ts          # Team ↔ AgentBridge UI/Agent 消息桥接
│   │   ├── TeamRepository.ts      # 团队实例 / 任务 / 模板 / DAG 持久化
│   │   └── TeamManager.ts         # 团队生命周期 / Worktree / 导入导出协调器
│   ├── ipc/                       # IPC 通信（模块化）
│   │   ├── index.ts
│   │   ├── sessionHandlers.ts
│   │   ├── agentHandlers.ts
...
│   │   ├── teamHandlers.ts        # ★ Team IPC 通道（创建 / DAG / 生命周期 / 模板 / 导入导出 / Worktree）
│   │   ├── telegramHandlers.ts
│   │   └── systemHandlers.ts
│   ├── storage/                   # 数据持久化
...
│   │       ├── TeamRepository.ts  # ★ 团队 / 成员 / 任务 / 消息 / 模板 / DAG 查询
│   │       └── ...
...
用户创建团队实例（选择模板 + 填入目标）
  ...
  → 可选：为非 Leader 角色创建独立 Git Worktree
  → 每个角色 Agent 启动后收到 System Prompt（角色职责 + 可用 MCP 工具）
  → TeamManager 计算任务 DAG、检测循环依赖、标记 ready batch
  ...
  → 所有事件通过 IPC 推送到渲染进程 → teamStore → 看板 / DAG / 消息面板实时更新
  → 用户可从 UI 发单播 / 广播消息、编辑任务、暂停 / 取消团队
  → SQLite 持久化团队实例 / 成员 / 任务 / 消息 / 模板；支持快照导出 / 导入
```

与 Claude 官方 Agent Teams 的对比优势：
- **多 Provider 混搭**：每个角色独立选择最适合的 AI（Claude 做架构、Gemini 读大文件、Codex 写代码）
- **完全本地运行**：数据不离开本机，无需云端调度服务
- **依赖可视化**：任务依赖 DAG + 执行批次视图，比纯任务列表更适合协同拆解
- **可视化实时追踪**：任务看板 + DAG + 消息流直观展示协作过程
- **模板化复用**：团队定义可保存为模板，一键复用到新任务
- **工程隔离能力**：可选 Worktree 隔离，让多成员并发写代码时更安全
