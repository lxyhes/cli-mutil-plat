...
export function registerTaskHandlers(deps: IpcDependencies): void {
  ...
  ipcMain.handle(IPC.WORKFLOW_START, async (_event, workflowConfig: any) => {
    try {
  ...
  ipcMain.handle(IPC.WORKFLOW_APPROVE_STEP, async (_event, executionId: string, stepId: string, approved: boolean) => {
    try {
      const success = deps.orchestrator.approveStep(executionId, stepId, approved)
      return { success }
    } catch (error: any) {
      return { success: false, error: error.message }
    }
  ...
  ipcMain.handle('orchestrator:start-workflow', async (_event, workflow: any, variables?: Record<string, string>) => {
    try {
      deps.orchestrator.registerWorkflow(workflow)
      const execution = deps.orchestrator.startWorkflow(workflow.id, variables)
      if (!execution) {
  ...
  ipcMain.handle('orchestrator:cancel-workflow', async (_event, executionId: string) => {
    try {
      const success = deps.orchestrator.cancelWorkflow(executionId)
      return { success }
    } catch (error: any) {
      return { success: false, error: error.message }
    }
  ...
  ipcMain.handle('orchestrator:get-execution', async (_event, executionId: string) => {
    try {
      const execution = deps.orchestrator.getExecution(executionId)
      if (!execution) return { success: false, error: 'Not found' }
      return {
        success: true,
        execution: {
          ...execution,
  ...
  ipcMain.handle('orchestrator:get-active-executions', async () => {
    try {
      return deps.orchestrator.getActiveExecutions().map(e => ({
        ...e,
        startedAt: e.startedAt.toISOString(),
        completedAt: e.completedAt?.toISOString()
      }))
    } catch (error) {
      console.error('[IPC] orchestrator:get-active-executions error:', error)
      return []
    }
  })
}
