...
如果信息不足，请告诉我需要提供什么额外信息。`,
  ...
  {
    id: 'builtin-cross-review',
    name: '多模型交叉审查',
    description: '同时使用 Claude + Codex 从不同角度审查代码，获取多维度分析',
    category: 'development',
    slashCommand: 'cross-review',
    type: 'orchestration',
    compatibleProviders: ['claude-code', 'codex'],
    orchestrationConfig: {
      steps: [
        {
          name: 'claude-review',
          providerId: 'claude-code',
          promptTemplate: `作为资深代码审查专家，请对以下代码进行全面审查：

{{user_input}}

重点关注：架构设计合理性、代码质量、潜在风险、最佳实践应用。
请给出具体的问题点和改进建议。`,
        },
        {
          name: 'codex-review',
          providerId: 'codex',
          promptTemplate: `请审查以下代码的实现细节和优化空间：

{{user_input}}

重点关注：算法效率、性能瓶颈、代码简洁性、边界情况处理。
给出具体的优化建议和替代实现方案。`,
        },
      ],
      mergeStrategy: 'concatenate',
      outputFormat: '请综合以上两个角度的审查意见，提炼出最关键的改进点。',
    },
    isInstalled: true,
    isEnabled: true,
    source: 'builtin',
    version: '1.0.0',
    author: 'ClaudeOps',
    tags: ['review', 'multi-model', 'orchestration'],
    createdAt: NOW,
    updatedAt: NOW,
  },
  {
    id: 'builtin-parallel-analysis',
    name: '并行多角度分析',
    description: '使用 Claude + Gemini 并行分析问题，综合得出更全面的见解',
    category: 'analysis',
    slashCommand: 'parallel-analysis',
    type: 'orchestration',
    compatibleProviders: ['claude-code', 'gemini-cli'],
    orchestrationConfig: {
      steps: [
        {
          name: 'claude-analysis',
          providerId: 'claude-code',
          promptTemplate: `请从技术实现和最佳实践角度深度分析以下问题：

{{user_input}}

给出详细的技术分析、实现建议和潜在风险评估。`,
        },
        {
          name: 'gemini-analysis',
          providerId: 'gemini-cli',
          promptTemplate: `请从系统设计和长期可维护性角度分析以下问题：

{{user_input}}

重点考虑：可扩展性、系统边界、技术选型权衡、长期维护成本。`,
        },
      ],
      mergeStrategy: 'concatenate',
    },
    ...
    version: '1.0.0',
    author: 'ClaudeOps',
    tags: ['analysis', 'multi-model', 'orchestration'],
    createdAt: NOW,
    updatedAt: NOW,
  },
]
