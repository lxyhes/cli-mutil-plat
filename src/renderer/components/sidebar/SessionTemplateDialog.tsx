/**
 * 会话模板创建对话框
 * 提供预设模板快速创建会话 + 自定义模板管理
 */
import { useState, useEffect } from 'react'
import {
  FileCode, FileText, Bug, PenTool, TestTube,
  RefreshCw, Plus, Trash2, X, Tag
} from 'lucide-react'
...
export default function SessionTemplateDialog({ onClose, onSelectTemplate }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div
        className="w-[560px] max-h-[80vh] bg-bg-secondary border border-border rounded-xl shadow-2xl flex flex-col"
        ...
        {showCreate && (
          <div className="px-4 py-3 border-b border-border bg-bg-tertiary/30 space-y-2">
            <div className="flex gap-2">
              ...
              <select className={`${inputCls} w-28`} value={newCategory} onChange={e => setNewCategory(e.target.value)}>
        ...
        <div className="px-4 py-2 border-b border-border flex gap-1.5 overflow-x-auto">
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-2.5 py-1 rounded-full text-[10px] whitespace-nowrap transition-colors ${
                selectedCategory === cat ? 'bg-accent-blue/15 text-accent-blue border border-accent-blue/40' : 'text-text-muted hover:text-text-secondary border border-transparent'
              }`}
            >
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {t.isBuiltIn && (
                      <span className="text-[9px] px-1 py-0.5 rounded bg-accent-purple/15 text-accent-purple">内置</span>
                    )}
                  </div>
                  {t.description && (
                    <p className="text-[10px] text-text-muted mt-0.5 line-clamp-2">{t.description}</p>
                  )}
                </div>
                {!t.isBuiltIn && (
                  <button
          ...
          {!loading && !filteredTemplates.length && (
            <div className="text-xs text-text-muted text-center py-8">
              {selectedCategory ? '该分类下暂无模板' : '暂无模板'}
            </div>
          )}
        </div>
      </div>
