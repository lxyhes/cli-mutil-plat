...
interface SessionState {
  ...
  sessionInitData: Record<string, { model: string; reasoningEffort?: string; tools: string[]; skills: any[]; mcpServers: any[]; availableModels: Array<{ id: string; name: string; description?: string }> }>  // SDK V2: 会话初始化数据
    ...
    set((state) => ({
      sessionInitData: {
        ...
        [sessionId]: {
          ...
          reasoningEffort: data.reasoningEffort ?? state.sessionInitData[sessionId]?.reasoningEffort ?? '',
