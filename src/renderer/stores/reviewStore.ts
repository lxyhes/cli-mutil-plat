...
export interface CodeReview {
  ...
  totalComments: number; criticalCount: number; createdAt: string
}
...
export const useReviewStore = create<ReviewState>((set) => ({
  reviews: [], comments: [], loading: false,
  ...
  startReview: async (params) => {
    try {
      ...
      return r?.success ? r.review : null
    } catch { return null }
  },

  resolveComment: async (commentId) => {
    try { await api()?.resolveComment(commentId) } catch { /* ignore */ }
  },
