...
export type PanelId = 'sessions' | 'explorer' | 'git' | 'dashboard' | 'team' | 'timeline' | 'stats' | 'mcp' | 'skills' | 'toolbox' | 'trending' | 'context' | 'memory' | 'checkpoint' | 'cost' | 'knowledge' | 'review' | 'battle' | 'replay' | 'arena' | 'context-budget'
...
/** 所有面板 ID（用于校验） */
const ALL_PANEL_IDS: PanelId[] = ['sessions', 'explorer', 'git', 'dashboard', 'team', 'timeline', 'stats', 'mcp', 'skills', 'toolbox', 'trending', 'context', 'memory', 'checkpoint', 'cost', 'knowledge', 'review', 'battle', 'replay', 'arena', 'context-budget']
